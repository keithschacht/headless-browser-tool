...
class TestUncheckTool < Minitest::Test
  ...
  def test_element_not_found
    tool = HeadlessBrowserTool::Tools::UncheckTool.new
  ...
  def test_checkbox_already_unchecked
    tool = HeadlessBrowserTool::Tools::UncheckTool.new
    Thread.current[:hbt_session_id] = "test-session"
    ...
    refute result[:is_checked]
  ensure
    Thread.current[:hbt_session_id] = nil
  end

  def test_checkbox_needs_unchecking_via_click
    tool = HeadlessBrowserTool::Tools::UncheckTool.new
    Thread.current[:hbt_session_id] = "test-session"
  ...
  def test_checkbox_click_failed_to_uncheck
    tool = HeadlessBrowserTool::Tools::UncheckTool.new
    Thread.current[:hbt_session_id] = "test-session"
    ...
    assert_equal "error", result[:status]
    assert_equal "Clicked element #my-checkbox but it did not change to unchecked state", result[:error]
  ensure
    Thread.current[:hbt_session_id] = nil
  end
